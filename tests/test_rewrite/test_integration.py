...
def setup_multi_snapshot_table(s3_client):
    ...
    upload_bytes(s3_client, manifest_key_3, make_manifest_avro([
        f"{SRC_PREFIX}/db/table/data/snap2-part-0.parquet",
        f"{SRC_PREFIX}/db/table/data/snap2-part-1.parquet",
    ]))
    upload_bytes(s3_client, manifest_key_4, make_manifest_avro([
    ...
    metadata = {
        ...
        "metadata-log": [
            {"timestamp-ms": 1699999000000, "metadata-file": f"{SRC_PREFIX}/db/table/metadata/v1.metadata.json"},
            {"timestamp-ms": 1700000000000, "metadata-file": f"{SRC_PREFIX}/db/table/metadata/v2.metadata.json"},
        ],
    }
    upload_bytes(
        s3_client,
...
def test_full_pipeline_rewrite_and_validate(s3_client):
    ...
    assert len(phase1_graph.manifests) == 2, "Phase 1 only loads current snapshot's manifests"
    ...
    assert len(full_original_graph.manifest_lists) == 2, "Full graph has 2 manifest lists"
    ...
    assert len(result.graph.manifest_lists) == 2, "Rewritten graph has 2 manifest lists (all snapshots)"
    ...
    # ----- Core assertions -----
    assert validation.passed is True, f"Validation should pass. Errors: {validation.errors}"
    assert validation.residual_prefix_count == 0, "No residual source prefix in any rewritten file"
    assert validation.structural_valid is True, "Rewritten metadata.json must be structurally valid"
    assert validation.errors == [], f"No validation errors expected, got: {validation.errors}"
    ...
    for key, ml_bytes in result.manifest_list_bytes.items():
        ...
        for rec in records:
            ...
            assert "s3a://" not in mp, f"s3a:// should be gone from manifest_path in {key}: {mp}"
    ...
    for key, m_bytes in result.manifest_bytes.items():
        ...
        for rec in records:
            ...
            assert "s3a://" not in fp, f"s3a:// should be gone from file_path in {key}: {fp}"
    ...
    assert b"s3a://" not in result.metadata_bytes, "metadata_bytes must not contain s3a://"
    for key, bytes_val in result.manifest_list_bytes.items():
        assert b"s3a://" not in bytes_val, f"manifest_list_bytes[{key}] must not contain s3a://"
    for key, bytes_val in result.manifest_bytes.items():
        assert b"s3a://" not in bytes_val, f"manifest_bytes[{key}] must not contain s3a://"
