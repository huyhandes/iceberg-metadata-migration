"""Local integration test: migrate Iceberg table seeded via Lakekeeper REST catalog.
...
@pytest.mark.rest
def test_rest_metadata_exists_on_minio(minio_client):
    ...
    assert len(objects) > 0, (
        ...
        "Did you run: uv run python infra/seed/seed_rest.py?"
    )
...
@pytest.mark.rest
def test_rest_migration_rewrites_paths(minio_client):
    """Run migration CLI against REST-seeded metadata and verify path rewriting."""
