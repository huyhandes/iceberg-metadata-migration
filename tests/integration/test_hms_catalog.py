"""Local integration test: migrate Iceberg table seeded via Hive Metastore.

Requires:
  - docker compose --profile hms up -d
  - uv run python infra/seed/seed_hms.py

Verifies the migration tool correctly rewrites paths in metadata produced
by a Hive Metastore catalog.
"""
...
@pytest.mark.hms
def test_hms_metadata_exists_on_minio(minio_client):
    ...
    assert len(objects) > 0, (
        ...
        "Did you run: uv run python infra/seed/seed_hms.py?"
    )
...
@pytest.mark.hms
def test_hms_migration_rewrites_paths(minio_client):
    """Run migration CLI against HMS-seeded metadata and verify path rewriting."""
